    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        return [
                ...
                "You are in the room with the peeling wallpaper",
                ][room_number]
    ...
    def say(self, argument):
        ...
        self.output_buffer = 'You say, "{}"'.format(
            argument
        )
    ...
    def route(self):
        ...
        input = self.input_buffer.lower()

        if "move north" in input:
            self.move("north")
        elif "move east" in input:
            self.move("east")
        elif "move south" in input:
            self.move("south")
        elif "move west" in input:
            self.move("west")
        elif "quit" in input:
            self.quit(None)
        else:
